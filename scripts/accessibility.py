...
cell_size = 5000
health_points = QgsVectorLayer(join(path, "crossing.geojson"), "health_points", "ogr")
number_of_points = 3
id_field = "osm_id"
...
def intersect_grid_and_roads(grid, roads):
    ...
    grid.startEditing()
    grid.addAttribute(QgsField("has_road", QVariant.String, len=10, prec=0))
    ...
    for cell in grid.getFeatures():
        ...
        grid.changeAttributeValue(cell.id(), intersection_index, has_intersection)
...
def centroids(layer):
    ...
    centroids_layer = create_memory_layer(
        "centroids", Qgis.Point, layer.crs(), layer.fields()
    )
...
def assign_cost_to_cells(network_graph, source, destination, id_field):
    ...
    routes = create_memory_layer("routes", Qgis.Line, destination.crs(), fields)
    routes.startEditing()

    source.startEditing()
    source.addAttribute(QgsField("destination_id", QVariant.Int, len=5, prec=0))
        ...
        if desination_id is None or isinstance(desination_id, QPyNullVariant):
            ...
            for health_point in nearest_health_points:
                try:
                    ...
                    geom, distance, _ = network_graph.route(source_geometry_point, p)
                except:
                    distance = -1

                if minimum_distance is None or (minimum_distance > distance >= 0):
                    minimal_geom = geom
