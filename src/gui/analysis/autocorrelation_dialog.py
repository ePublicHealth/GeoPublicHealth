...
from qgis.core import (
    ...
    QgsApplication, QgsClassificationMethod, QgsCoordinateTransformContext,
    QgsMessageLog
)
...
from GeoPublicHealth.src.core.exceptions import (
    ...
    FieldExistingException, NotANumberException
)
...
class CommonAutocorrelationDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        if not PYSAL_AVAILABLE:
            display_message_bar(
                tr("PySAL library is required for spatial autocorrelation analysis. "
        ...
        try:
            ...
            if hasattr(self, 'button_box_ok'):
                ...
                if cancel_button:
                    ...
                    cancel_button.clicked.connect(lambda: self.signalAskCloseWindow.emit(0))
            ...
            if hasattr(self, 'cbx_indicator_field') and hasattr(self, 'cbx_aggregation_layer'):
            ...
            if hasattr(self, 'label_library_info'):
                if GEOPANDAS_AVAILABLE:
                    self.label_library_info.setText(tr("Using modern GeoPandas approach"))
                elif PYSAL_AVAILABLE:
                    self.label_library_info.setText(tr("Using legacy PySAL approach"))
                else:
                    self.label_library_info.setText(tr("Required libraries not available"))
                    
        except Exception as e:
            display_message_bar(
                f"{tr('Error setting up UI:')} {str(e)}",
                level=Qgis.Critical
            )
            traceback.print_exc()
    ...
    def get_indicator_values_legacy(self, field):
        ...
        try:
            # Use context manager to ensure proper file closing
            with libpysal.io.open(self.admin_layer.source().replace('.shp', '.dbf')) as f:
    ...
    def calculate_moran_local(self, y, w):
        ...
        try:
            ...
            display_message_bar(
                f"{tr('Error calculating Local Moran\'s I:')} {str(e)}",
                level=Qgis.Critical
            )
            raise
    ...
    def create_output_features(self, file_writer, lm, sig_q):
        ...
        try:
            ...
            display_message_bar(
                f"{tr('Error creating output features:')} {str(e)}",
                level=Qgis.Critical
            )
            raise
    ...
    def create_output_layer(self, field):
        ...
        if not output_layer.isValid():
            ...
            raise GeoPublicHealthException(msg=tr("Failed to create valid output layer"))
        ...
        try:
            ...
            if is_gpkg:
                ...
                if result[0] != QgsVectorFileWriter.NoError:
                    display_message_bar(
                        tr("Failed to create significance layer"),
                        level=Qgis.Warning
                    )
                    return
