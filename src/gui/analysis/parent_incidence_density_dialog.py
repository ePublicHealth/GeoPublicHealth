...
from qgis.PyQt.QtWidgets import (
    QDialog, QDialogButtonBox, QTableWidgetItem, QApplication, 
    QFileDialog, QMessageBox, QVBoxLayout, QCheckBox
)
from qgis.PyQt.QtCore import (
    QSize, QVariant, Qt, pyqtSignal, QSettings
)
...
from qgis.core import (
    ...
    QgsField, QgsFields, QgsFeature, QgsFeatureRequest,
    QgsVectorLayer, QgsProject, QgsGeometry, QgsWkbTypes,
    QgsMapLayerProxyModel, QgsProcessingUtils, QgsFieldConstraints,
    QgsVectorFileWriter, QgsSpatialIndex, QgsUnitTypes,
    QgsGraduatedSymbolRenderer, QgsSymbol, QgsGradientColorRamp,
    QgsCoordinateTransformContext, QgsFieldProxyModel,
    QgsVectorDataProvider, QgsRendererRange, QgsClassificationRange
)
# QGIS GUI Imports
from qgis.gui import (
    QgsFieldComboBox, QgsMapLayerComboBox,
    QgsColorButton, QgsCollapsibleGroupBox
)
...
class IncidenceDensityTask(QgsTask):
    ...
    def run(self) -> bool:
        ...
        file_writer = None
        output_target_path = self.output_path + ('.gpkg' if self.driver_name == "GPKG" else '.shp')

        try:
            # --- Load Layers ---
            admin_layer = QgsVectorLayer(self.admin_layer_uri, "admin_layer_task", "ogr")
            ...
            if self.use_point_layer_flag and self.point_layer_uri:
                point_layer = QgsVectorLayer(self.point_layer_uri, "point_layer_task", "ogr")
            ...
            gpkg_layer_name = self.output_field_name.replace(' ', '_') if self.driver_name == "GPKG" else None
            ...
            if file_writer is None or file_writer.hasError():
                error_msg = file_writer.errorMessage() if file_writer else "Unknown error"
            ...
            if not self.use_area_flag:
                ...
                if index_population == -1:
                    raise FieldException(
                        tr("Population field '{}' not found.").format(self.population_field_name)
                    )
            ...
            intersection_field_idx = output_fields.lookupField(intersection_field_name) if self.add_intersections_flag else -1
            ...
            for i, admin_feature in enumerate(admin_features_iterator):
                ...
                if self.use_point_layer_flag and point_layer_index and point_layer:
                    ...
                    candidate_fids = point_layer_index.intersects(admin_geom.boundingBox())
                    if candidate_fids:
                        ...
                        for point_feat in point_layer.getFeatures(request):
                            ...
                            if point_geom and not point_geom.isEmpty() and point_geom.intersects(prepared_admin_geom):
                                count += 1
                    ...
                    if not (case_val is None or (isinstance(case_val, QVariant) and case_val.isNull())):
                        try:
                ...
                try:
                    if self.use_area_flag:
                        ...
                        if not (pop_val is None or (isinstance(pop_val, QVariant) and pop_val.isNull())):
                            try:
        ...
        # --- Error Handling ---
            ...
            if hasattr(self, 'output_path') and self.output_path:
                try:
                    output_target_path_on_error = self.output_path + ('.gpkg' if self.driver_name == "GPKG" else '.shp')
    ...
    def finished(self, result: bool):
        ...
        if result:
            ...
            error_msg = f"{tr('Task failed:')} {str(self.exception)}" if self.exception else tr('Task cancelled or failed.')
...
class IncidenceDensityDialog(QDialog):
    ...
    def _get_ui_elements(self):
        ...
        self.checkBox_addNbIntersections = getattr(self, 'checkBox_addNbIntersections', None)
        ...
        # Statistics widgets
        self.checkBox_incidence_runStats = getattr(self, 'checkBox_incidence_runStats', None)
    ...
    def _configure_conditional_widgets(self, widget_attr_name, filter_or_type, condition_to_enable, reset_func=None):
        ...
        label_widget = getattr(self, f"label_{widget_attr_name.replace('cbx_','')}", None)
        ...
        if widget:
            ...
            if condition_to_enable and self.cbx_aggregation_layer:
                if isinstance(widget, QgsMapLayerComboBox) and filter_or_type:
                    ...
                    try:
                        self.cbx_aggregation_layer.layerChanged.disconnect(widget.setLayer)
                    except TypeError:
                        pass
                        
                    try:
                        if reset_func:
                            self.cbx_aggregation_layer.layerChanged.disconnect(reset_func)
                    except TypeError:
                        pass
    ...
    def start_stats_task(self):
        ...
        # --- Get input values ---
        admin_layer = self.cbx_aggregation_layer.currentLayer() if self.cbx_aggregation_layer else None
        if not admin_layer:
            display_message_bar(tr("Please select an aggregation layer."), level=Qgis.Warning)
            return
        ...
        if self.use_point_layer:
            point_layer = self.cbx_case_layer.currentLayer() if self.cbx_case_layer else None
            if not point_layer:
                display_message_bar(tr("Please select a point case layer."), level=Qgis.Warning)
                return
            ...
            case_column_name = self.cbx_case_field.currentField() if self.cbx_case_field else None
            if not case_column_name:
                display_message_bar(tr("Please select a case field."), level=Qgis.Warning)
                return
                
        # Get population source (field or area)
        if not self.use_area:
            population_column_name = self.cbx_population_field.currentField() if self.cbx_population_field else None
            if not population_column_name:
                display_message_bar(tr("Please select a population field."), level=Qgis.Warning)
                return
        ...
        if not self.name_field:
            display_message_bar(tr("Output field name cannot be empty."), level=Qgis.Warning)
            return

        # Get other parameters
        add_nb_intersections = self.checkBox_addNbIntersections.isChecked() if self.checkBox_addNbIntersections else False
        ...
        try:
            ...
            display_message_bar(
                tr("Invalid ratio value entered (must be > 0)."),
                level=Qgis.Critical
            )
            return

        # --- Handle output path ---
        output_file_path_ui = self.le_output_filepath.text() if self.le_output_filepath else ""
        driver_name = "GPKG"
    ...
    def task_finished(self, success: bool, result_data: Optional[Dict[str, Any]], message: str):
        ...
        if success:
            ...
            gpkg_layer_name = self.name_field.replace(' ', '_') if driver_name == "GPKG" else None
            ...
            if layer_warnings:
                ...
                display_message_bar(tr("Processing finished successfully."), level=Qgis.Success)
            ...
            layer_uri = f"{output_full_path}|layername={gpkg_layer_name}" if driver_name == "GPKG" else output_full_path
            ...
            if self.output_layer and self.output_layer.isValid():
                ...
                if self.checkBox_incidence_runStats and self.checkBox_incidence_runStats.isChecked():
                ...
                display_message_bar(
                    tr("Output file may have been created, but failed to load as a layer:") + f"\n{layer_uri}",
                    level=Qgis.Critical
                )
            ...
            # Debug output
            output_base_path_on_fail = self.output_file_path if self.output_file_path else "Unknown"
            QgsMessageLog.logMessage(
    ...
    def _set_ui_running_state(self, running: bool):
        ...
        """
        # Update buttons
        ok_button = self.button_box_ok.button(QDialogButtonBox.Ok) if self.button_box_ok else None
        cancel_button = self.button_box_ok.button(QDialogButtonBox.Cancel) if self.button_box_ok else None
    ...
    def display_stats_and_plot(self, data: List[float], total_case: int):
        ...
        try:
            ...
            admin_layer_count = self.admin_layer.featureCount() if self.admin_layer and self.admin_layer.isValid() else 'N/A'
            ...
            if stats:
                ...
                for name in ['Min', 'Average', 'Max', 'Median', 'Range', 'Variance', 'Std Deviation']:
    ...
    def draw_plot(self, data: List[float]):
        ...
        try:
            ...
            if not isinstance(data, list) or not data:
                ...
                valid_indices = [i for i, val in enumerate(data) if val is not None and np.isfinite(val)]
    ...
    def add_symbology(self):
        ...
        try:
            ...
            if not method_id:
                display_message_bar(tr("Unknown classification mode, using EqualInterval."), level=Qgis.Warning)
            ...
            if classification_method:
                ...
                display_message_bar(tr("Failed to get classification method."), level=Qgis.Warning)
    ...
    def cancel_task_and_close(self):
        ...
        if self.current_task and self.current_task.isActive():
            display_message_bar(tr("Attempting to cancel running task..."), level=Qgis.Info)
    ...
    def closeEvent(self, event):
        ...
        if self.current_task and self.current_task.isActive():
            reply = QMessageBox.question(
                self,
                tr("Task Running"),
                tr("A task is currently running. Do you want to cancel it and close the window?"),
