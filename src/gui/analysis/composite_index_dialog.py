...
class CommonCompositeIndexDialog(QDialog):
    ...
    def setup_indicator_field(self):
        ...
        if not all(
            hasattr(self, attr)
            for attr in ["cbx_indicator_field", "cbx_aggregation_layer"]
        ):
            return
    ...
    def remove_item(self):
        ...
        if (
            hasattr(self, "cbx_list_indicators")
            and self.cbx_list_indicators.currentRow() >= 0
        ):
    ...
    def vector_direction(self) -> str:
        ...
        """
        if (
            hasattr(self, "radioButton_vector_positive")
            and self.radioButton_vector_positive.isChecked()
        ):
            return "+"
        return "-"
    ...
    def run_stats(self):
        ...
        if not all(
            hasattr(self, attr)
            for attr in ["cbx_aggregation_layer", "le_new_column", "le_output_filepath"]
        ):
            display_message_bar(
                tr("UI elements not properly initialized"), level=Qgis.Critical
            )
            return
    ...
    def check_indicators(self):
        ...
        if not self.use_point_layer and not self.use_area:
            if (
                not hasattr(self, "cbx_list_indicators")
                or self.cbx_list_indicators.count() == 0
            ):
    ...
    def setup_fields_and_output(self):
        ...
        if hasattr(self, "le_new_column"):
            self.name_field = (
                self.le_new_column.text() or self.le_new_column.placeholderText()
            )
    ...
    def add_fields_to_layer(self, fields):
        ...
        for indicator_selected in self.selected_indicators:
            ...
            fields.append(
                QgsField("Z" + indicator_selected[0], 6, "Real", 20, 10)
            )  # 6 = QVariant.Double

        # Add the composite index field
        fields.append(
            QgsField(self.name_field, 6, "Real", 20, 10)
        )  # 6 = QVariant.Double
    ...
    def calculate_composite_index(self, fields):
        ...
        for feature in self.admin_layer.getFeatures():
            attributes, composite_index_value = (
                self.calculate_attributes_and_composite_index(feature, stats)
            )
    ...
    def calculate_attributes_and_composite_index(
        self, feature, stats
    ...
    def add_symbology(self) -> None:
        ...
        try:
            ...
            if not registry:
                display_message_bar(
                    tr("Cannot access classification registry."), level=Qgis.Warning
                )
                return
            ...
            if classification_method:
                ...
                display_message_bar(
                    tr("Classification method not found."), level=Qgis.Warning
                )
